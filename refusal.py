...
best_refusal_dir = find_best_refusal_dir()[0]
my_model.apply_refusal_dirs([best_refusal_dir],layers=None)

with my_model: 
  ortho.apply_refusal_dir([my_new_precious_dir]) 
  print(my_model.mse_harmless(N=128)) 

ortho.test(N=16,batch_size = 4) 

ortho.generate("How much wood could a woodchuck chuck if a woodchuck could chuck wood?")